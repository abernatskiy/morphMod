...
generateOneScript() {
	...
	cat "${base_script}.py" | sed \
		...
		-e "s/'logParetoFrontPeriod': [0-9]*/'logParetoFrontPeriod': ${pareto_logging_period}/" > "autoscripts/${base_script}_N${num_segments}.py"
}
...
generateFromOneBase() {
	...
	generateOneScript "${base_script}" 3 125 1
	generateOneScript "${base_script}" 5 625 5
	generateOneScript "${base_script}" 10 2000 10
}
